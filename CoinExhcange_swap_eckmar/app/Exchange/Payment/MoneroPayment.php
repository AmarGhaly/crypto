...
use App\Exchange\Utility\MoneroConvert;
use App\Exchange\Utility\MoneroRPC\walletRPC;
...
class MoneroPayment implements Coin
{
    ...
    public function generateAddress(array $parameters = []): string {
        try{
            ...
            throw new AddressGenerationException('xmr', $exception->getMessage());
        }
    ...
    public function getBalance(array $parameters = []): float {
        try{
            ...
            throw new BalanceCheckingException('xmr',$exception->getMessage());
        }
    ...
    public function sendToAddress(string $toAddress, float $amount) {
        try{
            ...
            $tx = $this->monero->transfer(['address' => $toAddress, 'amount' => $amount]);
            return $tx;
        } catch (\Exception $exception){
            if (strpos($exception->getMessage(),'not enough unlocked money') == false){
                throw new WalletSendException($this->coinLabel(),$exception);
            }
            if (app()->environment() == 'local'){
                Log::info('not enough unlocked money');
            }
